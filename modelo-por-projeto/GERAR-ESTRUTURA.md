# Instruções para gerar um Work Brain (organizado por projeto/seção)
...
> cérebro de trabalho funcional e será entrevistado para preenchê-lo.

---

## Instruções de execução (para o assistente)

Você vai construir um repositório de memória de trabalho. Siga nesta ordem:
...
3. **Escreva cada arquivo** exatamente com o conteúdo especificado. Os blocos delimitados
   ...
   código internas.
4. **Rode `git init`** e faça o primeiro commit: `Abre o brain`.
...
Leia `mapa.md` na raiz deste repositório AGORA, antes de qualquer outra ação.
...
> Índice do cérebro. Se você é o assistente e acabou de abrir este repositório, este é o
...
```
/cerebro     ← acorda: contexto + painel + últimos commits
/rotina      ← (manhã) cockpit do dia
... trabalha ...
/salve       ← (fim) destila, regenera o painel, commita
```

## Skills

- `/cerebro` — boot enxuto. Lê `context/` + `estado-atual.md` + git log de 3 dias. Nada mais.
- `/rotina` — cockpit do dia: top 4-5, aging, cobranças.
- `/salve` — fim de sessão: caça gaps, **regenera o `estado-atual.md`**, commit + push.
...
- **Commits:** no imperativo, em lote via `/salve` — não a cada mudança.
...
Repositório **privado**. Nunca commitar credencial, senha ou token. Cuidado com dado
pessoal — na dúvida, anonimize ou deixe fora.
~~~~
...
# Coleta bruta e artefatos pesados
*.csv
*.parquet
*.sqlite
*.db
...
1. **Sincronizar:** `git pull --rebase`.
2. **Ler o contexto pessoal:** `context/quem-sou.md` e `context/pessoas.md`.
3. **Ler `estado-atual.md`** (núcleo do boot).
   ...
   - Data do cabeçalho com mais de 7 dias? Avise que pode estar stale e pergunte se
     reflete a realidade.
4. **Inspecionar o movimento recente:**
   ```
   git log --since="3 days ago" --pretty=format:"%h %an %ar %s" --stat
   ```
   O log já narra o que mudou. **Não abra os arquivos** pra descobrir.
5. **NÃO LER no boot:** nada de `projetos/`, `memory/` ou `referencia/`.
6. **Sintetizar o panorama** a partir do painel + git log. Entregar com clareza, **não
   regenerar do zero** — destilar já foi trabalho do `/salve`.
...
- HH:MM ago — commit msg curta — projetos tocados
...
- **`git pull` com mudança local:** `git stash` → `git pull` → `git stash pop`. Avise.
...
7. **Escrever `memory/sessions/<YYYY-MM-DD>.md`.** Já existe arquivo do dia? **Anexe nova
   seção** (manhã/tarde/noite). Nunca sobrescreva.
...
9. **🔒 GUARDAS antes de commitar.** Arquivo acima de 1 MB no stage **para o commit** até
   haver decisão explícita:

   ```bash
   git diff --cached --name-only | while read f; do [ -f "$f" ] && s=$(stat -c%s "$f") && [ "$s" -gt 1000000 ] && echo "GRANDE: $f"; done
   ```

   Coleta bruta de sistema carrega nome real de pessoa e texto livre de cliente, e
   `.gitignore` falha calado quando o caminho não bate. E confira o que está sendo
   **apagado**, não só o que entra:

   ```bash
   git diff --cached --name-status | grep "^D"
   ```

   Regra de ignore mal escrita apaga arquivo já commitado de outro assunto, e ninguém
   percebe até precisar dele.

10. **`git add` + `git commit` + `git push`.**

    🔴 **Se o push falhar, COMMITE MESMO ASSIM e avise alto.** Commit é local e não depende
    de rede; segurar tudo porque o push não foi é perder trabalho se a máquina reiniciar.
    Depois escreva no topo do `estado-atual.md`:
    > ⚠️ **N commit(s) local(is) não enviado(s)** desde <data>. Rodar `git push`.

    Tire a linha assim que o push sair.
...
12. **Pasta nova entra no `mapa.md` no mesmo `/salve`.**
...
`Sessao YYYY-MM-DD: <resumo curto>`
...
Existe? **Linke em vez de repetir.** Mudou? Corrija na fonte e confira quem apontava.

## Quando NÃO acionar

- Sessão de cinco minutos que não mudou nada. Não commite lixo.

## Falhas comuns

- **Push rejeitado:** `git pull --rebase`, resolve, push de novo.
- **Falha de auth:** avise pra checar a autenticação do git.
...
**anexa** nova seção (manhã / tarde / noite) em vez de sobrescrever.
...
⚠️ Cuidado com dado pessoal. Anonimize o que não for necessário.
~~~~
...
└── <assunto>/
    ...
    └── receitas.md        procedimentos e consultas que funcionaram
```
...
## Passo final: git e onboarding

Depois de escrever todos os arquivos:

```bash
git init
git add -A
git commit -m "Abre o brain"
```

Então avise que a estrutura está pronta e **conduza a entrevista de onboarding**:
