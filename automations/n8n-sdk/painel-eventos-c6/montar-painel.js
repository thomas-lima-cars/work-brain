/* ══════════════════════════════════════════════════════════════════════
   NÓ "Montar Painel" — Painel de Eventos C6

   Recebe as cinco tabelas do "MCP Consultas", confere cada pacote contra o
   próprio total e monta o HTML: KPIs, ranking de lojas e de representantes,
   eventos, veículos, lojas ofertantes, lojas novas e glossário.

   ─── ONDE CADA NÚMERO É CALCULADO ────────────────────────────────────
   O nó só prepara as linhas-base (veículo com suas ofertas resolvidas, e
   participação loja × evento). Tudo que depende do recorte escolhido na
   tela — KPIs, rankings, tabelas — sai de "calcula()", que é UMA função:
   ela roda aqui (para o "resumo" publicado na saída do nó, que as provas
   conferem) e viaja para a página pelo "toString()". Duas contas escritas
   separadas seriam duas contas divergindo.

   ─── AS REGRAS (decididas pelo Thomas em 2026-09-24) ─────────────────
   Vendido / arremate  negociação em status 2, 3 ou 7. O arrematante é a
                       loja da oferta vencedora ("offer_actual_id").
   Pontuação           1 ponto por veículo ofertado + 10 por arremate, por
                       evento. Vários lances no mesmo carro valem 1 ponto.
   Loja nova           loja do whitelabel 43 cadastrada dentro da janela do
                       evento (início das ofertas → fim).
   Representante       USUARIO_GP da planilha de lojas ativas do C6, cruzado
                       pelo CNPJ da loja. CNPJ fora da planilha ou sem
                       representante vira "Sem Representante". Mudou do key
                       account para a planilha em 2026-09-24.

   ⚠️ Sem barra invertida neste arquivo: ele viaja pro n8n como string.
   ══════════════════════════════════════════════════════════════════════ */
...
const VENDEDOR_C6 = 104754;       /* C6 Bank Lojista */
const WL_VITRINE_C6 = [43, 48];   /* canais servidos por compraveiculos.cars2you.com.br */
const STATUS_VENDA = [2, 3, 7];
const PONTOS_VEICULO = 1;
const PONTOS_ARREMATE = 10;

/* domínio completo em context/banco-de-dados/plataforma/dominios.md */
const STATUS_NOME = {
...
/* ── 1. ingestão: cinco pacotes, cada um conferido contra o seu total ── */
...
/* ── 1b. a planilha de representantes comerciais ──────────────────────
   Lida a cada run do SharePoint pelos nós "Baixar Representantes" e "Ler
   Representantes" (Extract from File, primeira aba — o nome da aba é o de
   um job do BigQuery e muda a cada exportação). Uma linha por loja do C6:
   NR_CNPJ, CD_LOJA, USUARIO_GP (o representante) e outras.

   Planilha que não veio, ou veio sem as colunas, DERRUBA o run: publicar o
   painel com todo mundo "Sem Representante" seria uma tela plausível e
   errada, que ninguém notaria.

   Medido na planilha de 2026-09-24 (30.718 linhas, 30.179 CNPJs): 35 CNPJs
   têm mais de um representante, todos com mais de um CD_LOJA — são filiais
   com o mesmo CNPJ. A loja do banco não diz qual filial é, então fica o
   representante que aparece em MAIS linhas; empate vira Sem Representante
   e conta nos avisos. Atribuir o errado é pior que deixar sem. */
...
/* as colunas variam POR LINHA: o Extract from File descarta célula vazia,
   então procurar numa linha só daria falso negativo */
let temCnpj = false;
let temRep = false;
...
/* do mais recente ao mais antigo: o JSON_ARRAYAGG não garante ordem */
const evPorId = {};
eventos.forEach((e) => { evPorId[e.id] = e; });

/* ── 3. ofertas, indexadas por negociação ───────────────────────────── */
const ofertaPorId = {};
const ofertasDaNeg = {};
T.ofertas.forEach((o) => {
...
/* ── 4. lojas ───────────────────────────────────────────────────────── */
/* o CNPJ fica aqui dentro: é chave do cruzamento, não sai no HTML */
...
/* ── 5. o link do anúncio ─────────────────────────────────────────────
   {host}/anuncio/veiculo/{marca}/{modelo}/{versao}/{uuid sem hifens}, tudo
   minúsculo e com encodeURIComponent por trecho — o padrão de 25/08 que o
   Radar de Estoque já usa (montar-html.js, "linkAnuncio"). Faltando um
   pedaço, sem link: melhor sem botão que botão que cai em lugar nenhum. */
...
/* ── 6. veículos (uma linha por negociação) ─────────────────────────── */
const vistos = {};
const veiculos = [];
T.veiculos.forEach((r) => {
  ...
  /* referência do deságio: o valor da venda quando vendeu, senão a última
     oferta. Sem oferta ou sem FIPE, não há deságio — e não zero. */
...
/* ── 7. participação loja × evento ─────────────────────────────────────
   [evento, loja, veículos ofertados, lances, arremates, última oferta
   antes do evento]. É a linha-base de todo ranking e da tabela de lojas. */
const negEv = {};
...
/* ── 8. avisos: só sobre o que esta coleta trouxe ───────────────────── */
const TEXTO_AVISO = {
...
/* fora da planilha e sem representante na planilha são a regra pedida, não
   defeito: vão para o resumo, não para os avisos */
...
/* ══════════════════════════════════════════════════════════════════════
   calcula(D, sel) — o recorte. "sel" = 'todos' ou o id de um evento.
   Roda aqui e na página. Nada de DOM aqui dentro.
   ══════════════════════════════════════════════════════════════════════ */
function calcula(D, sel) {
  ...
  /* janela de cadastro: a do evento; em "todos", do início do evento mais
     antigo até agora */
  let jIni = null;
  let jFim = null;
  ...
  /* lojas: soma a participação de cada evento do recorte */
  const porLoja = {};
  parts.forEach((p) => {
    ...
    a.arrem += p[4];
    a.eventos++;
    /* a última oferta de antes do PRIMEIRO evento do recorte em que ela
       apareceu — em "todos", é "antes do período" para aquela loja */
  ...
  /* representantes: soma a pontuação das lojas de cada um. "Sem
     Representante" é um grupo como os outros, mas vai sempre para o fim —
     é o que falta distribuir, não um concorrente no ranking */
  const porRep = {};
  rankLojas.forEach((r) => {
...
/* ══════════════════════════════════════════════════════════════════════
   APP(D) — o que roda no navegador. Viaja pelo toString(), junto com
   calcula(). Sem biblioteca: o painel abre offline.
   ══════════════════════════════════════════════════════════════════════ */
function APP(D, LOGO) {
  ...
  function render() {
    ...
    /* eventos: a lista inteira, sempre; o selecionado fica marcado */
  ...
  /* seletor de evento: todos + um por evento, do mais recente ao mais antigo */
  ...
  /* avisos: ícone no topo, só quando há algo */
  if (D.avisos.length) {
    const b = $('#b-aviso');
    b.hidden = false;
  ...
  /* tema: nasce no do arquivo, troca ao vivo, não é gravado */
  const ICO = {
...
/* ── 9. glossário: só a regra vigente ─────────────────────────────────── */
DADOS.glossario = [
...
/* ── 10. o HTML ──────────────────────────────────────────────────────── */
const CSS_PAINEL = [
  /* ".ctrl.so-icone" declara display e venceria o atributo hidden: o ícone
     de avisos apareceria em dia sem aviso nenhum */
  '[hidden]{display:none!important}',
  '.marca .logo{height:26px}',
  '.topo-acoes select.ctrl{max-width:340px;text-overflow:ellipsis}',
  /* no telefone o seletor ganha linha própria: ao lado do título ele comia
     143 dos 375px e o topo ia a 140px de altura. "display:contents" faz os
     controles virarem filhos da grade do topo, sem mudar o HTML */
...
/* dado dentro do <script>: "<" vira a sequência de escape unicode, para
   nenhum nome de loja fechar a tag. A barra é montada em tempo de execução
   porque este arquivo não pode ter barra invertida literal. */
...
/* ── 11. o resumo, que as provas conferem ─────────────────────────────── */
...
const resumo = {
  ...
  /* quantas lojas casaram com a planilha e por qual caminho — sem isso,
     "a planilha não tem a loja" e "o CNPJ não veio do banco" dariam a
     mesma tela, toda em Sem Representante */
...
/* A página não pode sair sem os blocos injetados: sem tema ela abre crua,
   sem logo abre com imagem quebrada — e nenhum dos dois dá erro. */
