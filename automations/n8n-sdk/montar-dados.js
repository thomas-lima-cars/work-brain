...
/* ══════════════════════════════════════════════════════════════════════
   SAIDA: o objeto DADOS como JSON, pra ser baixado e virar HTML fora daqui.

   Vai como STRING (nao como objeto) de proposito: o n8n serializa objetos
   grandes de forma imprevisivel entre nos, e o que interessa e o texto exato
   que o JSON.parse do navegador vai ler. Assim o que sai daqui e o que entra
   no HTML, byte a byte.
   ══════════════════════════════════════════════════════════════════════ */
const _json = JSON.stringify(DADOS);

/* provas que valem mais aqui do que na caixa de entrada de alguem */
if (DADOS.coorte.length === 0) throw new Error('coorte vazia — o relatorio nao teria base');
if (DADOS.meta.wls.length === 0) throw new Error('nenhum whitelabel na coorte_wl');

return [{ json: {
  dados_json: _json,
  _meta: {
    bytes: _json.length,
    whitelabels: DADOS.meta.wls.map(function (w) { return w[0] + ':' + w[1] + ':' + w[2]; }),
    safras: DADOS.coorte.length,
    avisos: DADOS.avisos,
    /* quais datasets vieram vazios — e o que o HTML vai marcar com selo */
    vazios: ['recWl','evLoginWl','evOfertaWl','evCompraWl','evCadDiaWl',
             'nrLoginWl','nrOfertaWl','nrCompraWl','ufCadWl','ufComWl',
             'topHistWl','topAnoWl','topAcessoWl','topOfertasWl','rxBuyerWl']
      .filter(function (k) {
        var v = DADOS[k];
        return !v || (v.length === 0) || (typeof v === 'object' && !v.length && !Object.keys(v).length);
      })
  }
} }];
