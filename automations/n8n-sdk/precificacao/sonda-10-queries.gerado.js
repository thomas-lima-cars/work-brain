/* ═══════════════════════════════════════════════════════════════════════════
   ESTUDO DE PRECIFICAÇÃO — SONDA 10: uma linha por venda, agora POR MODELO

   ⚠️ ARQUIVO GERADO. Não edite este nó direto — edite `amostra-modelo.config.json` e
   rode `node monta-sonda-10.js`. O bloco AMOSTRA abaixo é injetado de lá.
...
const AMOSTRA = {
  "nome": "top20-modelos-12m",
  "descricao": "Os 20 modelos mais vendidos em 12 meses, sem motos e pesados. Definido na execucao 51330.",
  "chave": "marca + nome do modelo, normalizados (UPPER/TRIM)",
  "excluir_categorias": ["Motocicleta","Caminhao","Onibus","Reboque","Semireboque"],
  "janela_meses": 12,
  "corte_razao": [0.2,1.2],
  "vendas_esperadas": 4582,
  "page": 50,
  "paginas_margem": 3,
  "modelos": [
    { "marca": "VOLKSWAGEN", "modelo": "GOL", "vendas": 507, "ids": 1 },
    { "marca": "CHEVROLET", "modelo": "ONIX", "vendas": 500, "ids": 1 },
    { "marca": "HYUNDAI", "modelo": "HB20", "vendas": 365, "ids": 1 },
    { "marca": "RENAULT", "modelo": "KWID", "vendas": 268, "ids": 1 },
    { "marca": "VOLKSWAGEN", "modelo": "POLO", "vendas": 246, "ids": 1 },
    { "marca": "FORD", "modelo": "KA", "vendas": 244, "ids": 1 },
    { "marca": "VOLKSWAGEN", "modelo": "VOYAGE", "vendas": 240, "ids": 1 },
    { "marca": "FIAT", "modelo": "ARGO", "vendas": 237, "ids": 1 },
    { "marca": "RENAULT", "modelo": "SANDERO", "vendas": 214, "ids": 1 },
    { "marca": "FIAT", "modelo": "STRADA", "vendas": 205, "ids": 2 },
    { "marca": "VOLKSWAGEN", "modelo": "SAVEIRO", "vendas": 198, "ids": 2 },
    { "marca": "FIAT", "modelo": "MOBI", "vendas": 171, "ids": 1 },
    { "marca": "FIAT", "modelo": "TORO", "vendas": 159, "ids": 2 },
    { "marca": "TOYOTA", "modelo": "COROLLA", "vendas": 157, "ids": 1 },
    { "marca": "FIAT", "modelo": "PALIO", "vendas": 155, "ids": 1 },
    { "marca": "JEEP", "modelo": "RENEGADE", "vendas": 154, "ids": 2 },
    { "marca": "FIAT", "modelo": "UNO", "vendas": 142, "ids": 1 },
    { "marca": "CHEVROLET", "modelo": "PRISMA", "vendas": 141, "ids": 1 },
    { "marca": "TOYOTA", "modelo": "HILUX", "vendas": 141, "ids": 2 },
    { "marca": "VOLKSWAGEN", "modelo": "FOX", "vendas": 138, "ids": 1 }
  ]
};
...
/* o filtro de motos e pesados, POR LINHA */
const SEM_PESADOS =
  " AND COALESCE(cat.name, 'Nao informada') NOT IN ('" +
  AMOSTRA.excluir_categorias.join("', '") + "')";

const RAZAO = '(o.price / a.fipe_price)';
/* ⚠️ NAO exigir code_fipe preenchido aqui. Essa condicao veio da versao em que
   o codigo FIPE ERA a chave de agrupamento — sem ele a linha nao tinha grupo.
   Agora a chave e o modelo, e exigir o codigo descartava 455 vendas (10% da
   amostra) sem motivo, deixando esta pagina com 4.127 linhas contra as 4.582
   dos graficos. Duas paginas que dizem ser a mesma amostra tem de ter o MESMO
   filtro — a prova no fim do monta-sonda-10.js compara os dois. */
const UTIL = ' AND o.price > 0 AND a.fipe_price > 0';
const CORTE =
  ' AND ' + RAZAO + ' >= ' + AMOSTRA.corte_razao[0] +
  ' AND ' + RAZAO + ' <= ' + AMOSTRA.corte_razao[1];

/* o recorte da amostra: os 20 modelos, por marca|modelo.
   Nao por model_id: o catalogo reparte o mesmo modelo em ids diferentes
   (sonda 7 achou pelo menos 50 nomes assim), e por id a Strada viraria dois
   grupos de 172 e 33. */
const CHAVES = AMOSTRA.modelos.map((x) => x.marca + '|' + x.modelo);
const ESCOPO =
  " AND CONCAT(" + MARCA + ", '|', " + MODELO + ") IN ('" + CHAVES.join("', '") + "')";

const FILTRO = ' WHERE ' + VENDA + JANELA + UTIL + CORTE + SEM_PESADOS + ESCOPO;
