...
const BASE = "SELECT u.id as user_id FROM users u INNER JOIN user_whitelabels uw ON uw.user_id = u.id AND uw.whitelabel_id = 43 WHERE u.deleted_at IS NULL AND u.internal_user = 0 AND u.email NOT REGEXP '(@cars2you|@datapage|@icarros|@itau|@teste|@dnr[.]com[.]br|@maisquecliente|@dealersclub|@c6bank[.]com)'";
...
Q.push({ queryName: 'kpi', database: 'cars2you_production', sql:
  "SELECT " +
  "(SELECT COUNT(*) FROM (" + BASE + ") t) as total_base, " +
  "(SELECT COUNT(DISTINCT ua.user_id) FROM user_access ua WHERE ua.user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as com_login, " +
  "(SELECT COUNT(DISTINCT o.buyer_user_id) FROM offers o WHERE o.deleted_at IS NULL AND o.buyer_user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as ofertantes, " +
  "(SELECT COUNT(*) FROM offers o WHERE o.deleted_at IS NULL AND o.buyer_user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as total_ofertas, " +
  "(SELECT COUNT(DISTINCT o.buyer_user_id) FROM advertisement_negotiations an INNER JOIN offers o ON o.id=an.offer_actual_id INNER JOIN advertisements a ON a.id=an.advertisement_id WHERE an.status IN (2,3,7) AND o.buyer_user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as compradores, " +
  "(SELECT COUNT(DISTINCT an.id) FROM advertisement_negotiations an INNER JOIN offers o ON o.id=an.offer_actual_id INNER JOIN advertisements a ON a.id=an.advertisement_id WHERE an.status IN (2,3,7) AND o.buyer_user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as negociacoes, " +
  "(SELECT SUM(o.price) FROM advertisement_negotiations an INNER JOIN offers o ON o.id=an.offer_actual_id INNER JOIN advertisements a ON a.id=an.advertisement_id WHERE an.status IN (2,3,7) AND o.buyer_user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as volume, " +
  "(SELECT MAX(an.finish_date_offer) FROM advertisement_negotiations an INNER JOIN offers o ON o.id=an.offer_actual_id INNER JOIN advertisements a ON a.id=an.advertisement_id WHERE an.status IN (2,3,7) AND o.buyer_user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as ultima_compra, " +
  "(SELECT COUNT(DISTINCT an.id) FROM advertisement_negotiations an INNER JOIN offers o ON o.id=an.offer_actual_id INNER JOIN advertisements a ON a.id=an.advertisement_id WHERE an.status=7 AND o.buyer_user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as status7, " +
  "(SELECT COUNT(DISTINCT an.id) FROM advertisement_negotiations an INNER JOIN offers o ON o.id=an.offer_actual_id INNER JOIN advertisements a ON a.id=an.advertisement_id WHERE an.status=2 AND o.buyer_user_id IN (SELECT user_id FROM (" + BASE + ") bb)) as status2"
});

Q.push({ queryName: 'situacao', database: 'cars2you_production', sql:
  "SELECT u.situation, COUNT(*) as qtd FROM users u WHERE u.id IN (SELECT user_id FROM (" + BASE + ") bb) GROUP BY u.situation ORDER BY u.situation"
});
...
return Q.map(function(q){ return { json: q }; });